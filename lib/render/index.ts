...
import { loadDevaMark } from './assets'
...
async function draw(ctx: Ctx2D, spec: CardSpec): Promise<void> {
  switch (spec.format) {
    case 'pfp':
      return drawPfp(ctx, spec)
    case 'id': {
      const devaMark = await loadDevaMark()
      return drawBuilderId(ctx, spec, devaMark)
    }
    case 'crew':
